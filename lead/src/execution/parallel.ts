...
import { PROGRESS_FILE, LEAD_DIR } from "../config/loader.ts";
...
async function runAgentInWorktree(
	...
	try {
		...
		// Ensure .lead/ exists in worktree
		const leadDir = join(worktreeDir, LEAD_DIR);
...
async function runAgentInSandbox(
	...
	try {
		...
		// Ensure .lead/ exists in sandbox
		const leadDir = join(sandboxDir, LEAD_DIR);
		if (!existsSync(leadDir)) {
			mkdirSync(leadDir, { recursive: true });
		}
...
export async function runParallel(
	...
	if (!skipMerge && !dryRun && completedBranches.length > 0) {
		...
		try {
			...
			if (hasChanges) {
				await git.stash(["push", "-u", "-m", "lead-merge-stash"]);
				stashed = true;
