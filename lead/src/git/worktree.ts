...
export async function createAgentWorktree(
	...
	const branchName = `lead/agent-${agentNum}-${uniqueId}-${slugify(taskName)}`;
...
export function getWorktreeBase(workDir: string): string {
	const worktreeBase = join(workDir, ".lead-worktrees");
...
export async function listWorktrees(workDir: string): Promise<string[]> {
	...
	for (const line of lines) {
		if (line.startsWith("worktree ") && line.includes(".lead-worktrees")) {
