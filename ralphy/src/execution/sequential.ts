...
export async function runSequential(options: ExecutionOptions): Promise<ExecutionResult> {
	...
	while (true) {
		...
		const repoHadCommits = await hasCommits(workDir);
		const startHead = repoHadCommits ? await getCurrentCommit(workDir) : "";
		...
		if (dryRun) {
			...
			try {
				...
				if (aiResult.success) {
					...
					const repoChanged = await repoChangedSince(startHead, repoHadCommits, workDir);
					if (!repoChanged) {
						const reason = `No repository changes detected for task "${task.title}", skipping completion.`;
						logWarn(reason);
						result.tasksFailed++;
						notifyTaskFailed(task.title, reason);
					} else {
						// Mark task complete
