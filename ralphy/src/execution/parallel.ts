...
interface ParallelAgentResult {
	...
	usedSandbox?: boolean;
	/** Starting commit information for change detection */
	startHead?: string;
	repoHadCommits?: boolean;
}

async function repoChangedSince(startHead: string, repoHadCommits: boolean, dir: string): Promise<boolean> {
	const workingChanges = await hasUncommittedChanges(dir);
	if (workingChanges) return true;

	const repoHasCommits = await hasCommits(dir);
	const currentHead = repoHasCommits ? await getCurrentCommit(dir) : "";

	if (!repoHadCommits && repoHasCommits) return true;
	if (repoHadCommits && repoHasCommits && startHead !== currentHead) return true;
...
async function runAgentInWorktree(
	...
	try {
		...
		const repoHadCommits = await hasCommits(worktreeDir);
		const startHead = repoHadCommits ? await getCurrentCommit(worktreeDir) : "";
		...
		return { task, agentNum, worktreeDir, branchName, result, startHead, repoHadCommits };
	} catch (error) {
...
async function runAgentInSandbox(
	...
	try {
		...
		const repoHadCommits = await hasCommits(sandboxDir);
		const startHead = repoHadCommits ? await getCurrentCommit(sandboxDir) : "";
...
export async function runParallel(
	...
	while (true) {
		...
		for (const agentResult of results) {
			...
			const startHead = agentResult.startHead ?? "";
			const repoHadCommits = agentResult.repoHadCommits ?? false;
			const noChangeReason = `No repository changes detected for task "${task.title}", skipping completion.`;
			...
			if (!failureReason && aiResult?.success && agentUsedSandbox && worktreeDir) {
				try {
					...
					if (modifiedFiles.length > 0) {
						...
						repoChanged = commitResult.success && commitResult.filesCommitted > 0;
						...
						repoChanged = false; // Explicit for clarity
					}
				} catch (commitErr) {
			...
			if (!failureReason && aiResult?.success && !agentUsedSandbox) {
				repoChanged = await repoChangedSince(startHead, repoHadCommits, worktreeDir || workDir);
			}
			...
			if (failureReason) {
				...
				if (!repoChanged) {
					logWarn(noChangeReason);
					result.tasksFailed++;
					failureReason = noChangeReason;
					notifyTaskFailed(task.title, noChangeReason);
				} else {
					await taskSource.markComplete(task.id);
			...
			if (worktreeDir) {
				if (agentUsedSandbox) {
					const shouldPreserveSandbox =
						preserveSandbox || (failureReason !== undefined && failureReason !== noChangeReason);
