...
/**
 * Get the current commit hash (empty string if repo has no commits)
 */
export async function getCurrentCommit(workDir = process.cwd()): Promise<string> {
	const git: SimpleGit = simpleGit(workDir);
	try {
		return await git.revparse(["HEAD"]);
	} catch {
		// Repository may be unborn (no commits yet)
		return "";
	}
}
